import { TranscriptFormatter, FormatOptions, FormatTemplate } from "../src/transcript-formatter";
...
describe("TranscriptFormatter", () => {
	const mockTranscriptResponse: TranscriptResponse = {
		...
		lines: [
			...
			{ text: "for comprehensive testing", offset: 13300, duration: 4000 }
		]
	};
	...
	describe("format", () => {
		describe("minimal template", () => {
			it("should return plain text without timestamps", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, {
					timestampMod: 5,
					template: FormatTemplate.MINIMAL
				});
				expect(result).toBe("Hello world This is a test of the transcript formatting system with multiple lines for comprehensive testing");
			});
			...
			it("should join transcript lines with spaces", () => {
				...
				const result = TranscriptFormatter.format(simpleTranscript, testUrl, {
					timestampMod: 1,
					template: FormatTemplate.MINIMAL
				});
			...
			it("should trim extra whitespace", () => {
				...
				const result = TranscriptFormatter.format(spacedTranscript, testUrl, {
					timestampMod: 1,
					template: FormatTemplate.MINIMAL
				});
		...
		describe("standard template", () => {
			it("should include clickable timestamps", () => {
				...
				expect(result).toContain("[00:00](https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=0)");
				expect(result).toContain("[00:05](https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=5)");
				expect(result).toContain("[00:10](https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=10)");
			});
			...
			it("should format timestamps correctly", () => {
				...
				const result = TranscriptFormatter.format(timestampTranscript, testUrl, {
					timestampMod: 1,
					template: FormatTemplate.STANDARD
				});
			...
			it("should include proper YouTube links with time parameters", () => {
				...
				expect(result).toContain("&t=5) of the transcript formatting system");
				expect(result).toContain("&t=10) with multiple lines for comprehensive testing");
			});
			...
			it("should handle URLs with existing parameters", () => {
				const urlWithParams = "https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=PLtest";
				const result = TranscriptFormatter.format(mockTranscriptResponse, urlWithParams, {
					timestampMod: 2,
					template: FormatTemplate.STANDARD
				});
		...
		describe("rich template", () => {
			...
			it("should include source URL", () => {
				...
				expect(result).toContain("**Source**: https://www.youtube.com/watch?v=dQw4w9WgXcQ");
			});
			...
			it("should handle empty title gracefully", () => {
				const noTitleTranscript: TranscriptResponse = {
					title: "",
					lines: [{ text: "Test content", offset: 0, duration: 1000 }]
				};
				const result = TranscriptFormatter.format(noTitleTranscript, testUrl, {
					timestampMod: 1,
					template: FormatTemplate.RICH
				});
		...
		describe("template handling", () => {
			...
			it("should use default template when not specified", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, defaultOptions);
			...
			it("should apply timestampMod correctly across all templates", () => {
				const optionsWithMod: FormatOptions = { timestampMod: 2, template: FormatTemplate.STANDARD };
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, optionsWithMod);
		...
		describe("edge cases and error handling", () => {
			it("should handle empty transcript", () => {
				...
				const result = TranscriptFormatter.format(emptyTranscript, testUrl, {
					timestampMod: 5,
					template: FormatTemplate.STANDARD
				});
				expect(result).toBe("");
			...
			it("should handle single line transcript", () => {
				const singleLineTranscript: TranscriptResponse = {
					title: "Single Line",
					lines: [{ text: "Only one line", offset: 0, duration: 2000 }]
				};
				const result = TranscriptFormatter.format(singleLineTranscript, testUrl, {
					timestampMod: 1,
					template: FormatTemplate.MINIMAL
				});
			...
			it("should handle null transcript response", () => {
				const result = TranscriptFormatter.format(null as any, testUrl, { timestampMod: 5 });
				expect(result).toBe("");
			});

			it("should handle undefined transcript response", () => {
				const result = TranscriptFormatter.format(undefined as any, testUrl, { timestampMod: 5 });
				expect(result).toBe("");
			});

			it("should handle null URL", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, null as any, { timestampMod: 5 });
				expect(result).toContain("Hello world This is a test"); // Should still format text
			});

			it("should handle empty URL", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, "", { timestampMod: 5 });
				expect(result).toContain("Hello world This is a test"); // Should still format text
			});

			it("should handle zero timestampMod", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, { timestampMod: 0 });
				// Should default to showing every line or handle gracefully
				expect(result).toContain("Hello world");
			});

			it("should handle negative timestampMod", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, { timestampMod: -1 });
				// Should handle gracefully
				expect(result).toContain("Hello world");
			});

			it("should handle timestampMod larger than transcript length", () => {
				const result = TranscriptFormatter.format(mockTranscriptResponse, testUrl, { timestampMod: 100 });
