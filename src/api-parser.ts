...
function generateTranscriptParams(videoId: string, useAsrStyle: boolean, field6Value: number, lang: string = "en"): string {
...
export function generateAlternativeTranscriptParams(videoId: string, lang: string = "en"): string[] {
	...
	return variations.map(variant => 
		generateTranscriptParams(videoId, variant.useAsrStyle, variant.field6Value, lang)
	);
}
...
export function extractParamsFromPage(htmlContent: string): string | null {
	...
	try {
		...
		console.log("🔧 DEBUG: Parsing ytInitialData JSON (length:", ytInitialDataString.length, "chars)");
		
		const ytInitialData = JSON.parse(ytInitialDataString);
		
		// Recursively search for getTranscriptEndpoint in the object
		function findGetTranscriptEndpoint(obj: any, path = "", depth = 0): string | null {
			...
			if (depth === 0) {
				console.log("🔍 DEBUG: Starting recursive search for getTranscriptEndpoint...");
			}
			...
			if (obj.getTranscriptEndpoint && obj.getTranscriptEndpoint.params) {
				...
				console.log(`📏 DEBUG: Params length: ${obj.getTranscriptEndpoint.params.length} characters`);
				return obj.getTranscriptEndpoint.params;
			}
			
			// Look for any key containing "transcript" for debugging
			if (depth < 3) { // Only log for shallow depths to avoid spam
				for (const key of Object.keys(obj)) {
					if (key.toLowerCase().includes('transcript') || key.toLowerCase().includes('engagement')) {
						console.log(`🔍 DEBUG: Found transcript/engagement-related key: "${key}" at path: ${path} (depth: ${depth})`);
					}
				}
			}
...
function extractVisitorData(htmlContent: string): string | null {
	...
	const visitorDataMatch = htmlContent.match(/"visitorData"\s*:\s*"([^"]+)"/)
...
export function parseVideoPageWithFallbacks(
...
): { title: string; transcriptRequests: Array<{ url: string; headers: Record<string, string>; body: string }> } {
	...
	console.log("🔍 DEBUG: Params found on video page:", pageParams || "NOT FOUND");
	
	if (pageParams) {
		console.log("📏 DEBUG: Page params length:", pageParams.length, "characters");
		console.log("🔍 DEBUG: Page params preview:", pageParams.substring(0, 100) + "...");
	}

	// Generate all possible parameter combinations
	const generatedParams = generateAlternativeTranscriptParams(videoId, config?.lang || "en");
	console.log("🔧 DEBUG: Generated", generatedParams.length, "params variations for video ID:", videoId);
	...
	if (pageParams) {
		...
		generatedParams.forEach((generatedParam, index) => {
			if (generatedParam === pageParams) {
				console.log(`🎯 MATCH! Page params identical to generated variation ${index + 1}`);
				foundMatch = true;
			}
		});
		
		if (!foundMatch) {
			console.log("🔄 DEBUG: Page params are DIFFERENT from all generated variations - this could be video-specific!");
		}
	}
			...
			context: {
				...
				adSignalsInfo: {
					params: [
						...
						{ key: "bih", value: "847" },
						{ key: "biw", value: "2504" },
						{ key: "brdim", value: "0,0,0,0,2520,0,2520,847,2520,847" },
						{ key: "vis", value: "1" },
						{ key: "wgl", value: "true" },
