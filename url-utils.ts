const MarkdownUrlPattern = /\[([^\[\]]*)\]\((https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})\)/gi;
...
function _cursorWithinBoundaries(cursorPosition: number, startIndex: number, length: number): boolean {
	let endIndex = startIndex + length;

	return startIndex <= cursorPosition && cursorPosition <= endIndex;
}

export function getUrlFromText(lineText: string, cursorPosition: number): [number, number] {

	// First check if we're in a link
	let linksInLine = lineText.matchAll(MarkdownUrlPattern);

	for (let match of linksInLine) {
		if (_cursorWithinBoundaries(cursorPosition, match.index ?? 0, match[0].length)) {
